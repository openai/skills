...
Vibe UI is a local-first DESIGN.md workflow for web UI generation. It helps Codex choose an inspired visual direction, apply the selected `DESIGN.md`, generate page prompts, and run a lightweight consistency pass after implementation.
...
Run from this skill directory, or call the script by absolute path:
...
- If the user names a style, run `use <design_id>` in the project root. This writes `DESIGN.md`, `DESIGN.generated.md`, and `.vibe-ui/current-design.json`.
...
The full standalone Vibe UI project also publishes a larger offline bundle. This OpenAI catalog contribution keeps only the compact curated set to reduce review and install size.
