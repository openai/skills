...
metadata:
  short-description: Read-only Sentry observability
---
...
When running from the repo root, invoke it via `<path-to-skill>/scripts/sentry_api.py`.

### 1) List issues (ordered by most recent)

```bash
python3 <path-to-skill>/scripts/sentry_api.py \
  list-issues \
  --org {your-org} \
