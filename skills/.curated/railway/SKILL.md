...
description: >
  ...
  when the user asks to deploy, host, or manage infrastructure on Railway.
---

# Railway

Operate Railway infrastructure: create projects, provision services and databases, deploy code, configure environments and variables, manage domains, troubleshoot failures, check status and metrics, and query Railway docs.

## Prerequisites

- When sandboxing blocks network calls, rerun with `sandbox_permissions=require_escalated`.
- Deployments might take a few minutes. Use appropriate timeout values.
...
If deployment or CLI commands fail due to network issues (timeouts, DNS errors, connection resets), rerun with escalated permissions (`sandbox_permissions=require_escalated`). Deployments require network access that sandboxing may block.
