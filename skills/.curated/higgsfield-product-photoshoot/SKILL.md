...
description: >-
  Generate brand-quality product images via the Higgsfield CLI product-photoshoot
  prompt enhancement on GPT Image 2. Use for product photos, studio shots,
  lifestyle scenes, Pinterest pins, hero banners, social carousels, ad creatives,
  virtual try-on, conceptual product imagery, and restyles. Not for video ads,
...
1. If `higgsfield` is not on `$PATH`, ask the user whether to install the Higgsfield CLI with the official installer:
   ```bash
