...
description: Diagnoses why Codex skills fail to trigger by checking descriptions, keywords, naming mismatches, and conflicts so the right skill activates.
metadata:
  short-description: Debugs why Codex skills are not triggering
---
...
Use this skill when:
- A skill exists but Codex never invokes it automatically
- You need to say the skill name explicitly to trigger it
- Codex says "skill doesn't exist" but you know it's installed
...
**Why it matters**: Specific descriptions help Codex understand exactly when to use the skill.
